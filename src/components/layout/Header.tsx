      ...
      <div className="container-fluid">
        <div className="flex items-center justify-between min-h-[60px] py-2">
          ...
          <nav className="hidden lg:flex items-center gap-7">
            <Link
              href={ROUTES.HOME}
              className="text-sm text-text-primary hover:text-primary transition-colors font-medium"
            >
              Home
            </Link>
            <div className="relative group">
              <button className="text-sm text-text-primary hover:text-primary transition-colors flex items-center gap-1 font-medium">
                Shop
      ...
      <div className={`fixed top-0 left-0 h-full w-[300px] max-w-[88vw] bg-white z-50 shadow-2xl flex flex-col lg:hidden transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="relative bg-gradient-to-r from-[#1a0a0a] to-[#3d1515] px-5 py-5 flex items-center justify-between flex-shrink-0">
          <Link href={ROUTES.HOME} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-bold text-base leading-tight">Mahek Sarees</p>
              <p className="text-white/60 text-[11px] mt-0.5">Premium Ethnic Wear</p>
            </div>
          </Link>
          <button
            onClick={() => setIsMobileMenuOpen(false)}
        ...
        <nav className="flex-1 overflow-y-auto">
          ...
          <div className="px-4 pt-4 pb-2">
            <Link
              href={ROUTES.HOME}
              onClick={() => setIsMobileMenuOpen(false)}
              className="flex items-center justify-between px-3 py-3 rounded-xl text-sm font-semibold text-gray-800 hover:bg-rose-50 hover:text-rose-700 transition-all group"
            >
              <div className="flex items-center gap-3">
          ...
          <div className="px-4 pb-2">
            <div className="mb-2 px-3">
              <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Shop by Category</p>
            </div>
          ...
          <div className="px-4 pb-4">
            <div className="mb-2 px-3">
              <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Explore</p>
            </div>
            <div className="space-y-1">
              {[
          ...
          <div className="flex items-center justify-between">
            ...
            {isAuth ? (
              ...
              <Link
                href={ROUTES.LOGIN}
                onClick={() => setIsMobileMenuOpen(false)}
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-rose-600 to-pink-600 text-white text-xs font-semibold shadow-sm hover:shadow-md transition-shadow"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
        ...
        <div className={`relative bg-white shadow-2xl transition-transform duration-300 ${isSearchOpen ? "translate-y-0" : "-translate-y-4"}`}>
          <div className="container-fluid py-5 sm:py-6">
            ...
            >
              ...
              <button
                type="submit"
                className="flex-shrink-0 flex items-center gap-2 px-5 sm:px-6 py-3 sm:py-3.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-sm font-semibold shadow-sm hover:shadow-md transition-all duration-200"
              >
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              {["Banarasi Saree", "Bridal Lehenga", "Rajputi Poshak", "Silk Saree", "Party Wear"].map((tag) => (
                <button
                  ...
                  className="px-3 py-1 text-xs bg-gray-100 hover:bg-rose-50 hover:text-rose-700 text-gray-600 rounded-full border border-gray-200 hover:border-rose-200 transition-all"
                >
                  {tag}
                </button>
              ))}
